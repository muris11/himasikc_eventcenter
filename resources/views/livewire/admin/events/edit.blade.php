        ...
        <div class="bg-surface rounded-3xl shadow-sm border border-border overflow-hidden">
            <div class="p-6 lg:p-8 bg-surface-secondary/30 border-b border-border">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-3xl font-bold text-text-main font-heading tracking-tight">Edit Event</h1>
                        <p class="text-text-muted mt-2">Perbarui informasi event di bawah ini.</p>
                    </div>
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $is_active ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-surface-secondary text-text-muted border border-border' }}">
                        <span class="w-2 h-2 rounded-full {{ $is_active ? 'bg-green-500' : 'bg-text-muted/50' }} mr-2"></span>
                ...
                <form wire:submit="save" class="space-y-8">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="col-span-2">
                            ...
                            <input type="text" wire:model="title" id="title-input" x-data
                                x-init="$el.value = '{{ $title }}'"
                        ...
                        <div class="col-span-2">
                            ...
                            <div class="flex rounded-xl shadow-sm">
                                ...
                                <input type="text" wire:model="slug" id="event-slug-input" x-data
                                    x-init="$el.value = '{{ $slug }}'"
                        ...
                        <div>
                            ...
                            <input type="date" wire:model="date" x-data x-init="$el.value = '{{ $date }}'"
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="location" x-data x-init="$el.value = '{{ $location }}'"
                                class="w-full px-4 py-3 border border-border rounded-xl shadow-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors placeholder-text-muted/50 bg-surface text-text-main">
                            @error('location')
                                <p class="mt-2 text-sm text-red-500 flex items-center animate-fade-in-up">
                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <select wire:model="category_id"
                                ...
                                @foreach (\App\Models\EventCategory::all() as $category)
                                    <option value="{{ $category->id }}"
                                        {{ $this->category_id == $category->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select wire:model="participant_type"
                                class="w-full px-4 py-3 border border-border rounded-xl shadow-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors bg-surface text-text-main">
                                <option value="all" {{ $participant_type == 'all' ? 'selected' : '' }}>Semua
                                    Peserta</option>
                                <option value="mahasiswa"
                                    {{ $participant_type == 'mahasiswa' ? 'selected' : '' }}>
                                    Mahasiswa</option>
                                <option value="umum" {{ $participant_type == 'umum' ? 'selected' : '' }}>
                                    Umum</option>
                            </select>
                        ...
                        <div class="col-span-2">
                            ...
                            <input type="text" wire:model="organizer" x-data x-init="$el.value = '{{ $organizer }}'"
                                class="w-full px-4 py-3 border border-border rounded-xl shadow-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors placeholder-text-muted/50 bg-surface text-text-main">
                            @error('organizer')
                                <p class="mt-2 text-sm text-red-500 flex items-center animate-fade-in-up">
                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="col-span-2">
                            ...
                            <div class="relative">
                                ...
                                <input type="url" wire:model="registration_link" x-data
                                    x-init="$el.value = '{{ $registration_link }}'"
                        ...
                        <div class="col-span-2" x-data="{ imagePreview: null }">
                            ...
                            <div
                                ...
                                <div class="space-y-2 text-center">
                                        ...
                                        <template x-if="!imagePreview">
                                            <div class="flex flex-col items-center">
                                                ...
                                                <div class="flex text-sm text-text-muted">
                                                    ...
                                                    <p class="pl-1">untuk mengganti gambar saat ini</p>
                                                </div>
                                    ...
                                    <input x-ref="fileInput" id="event-edit-image" wire:model="image" type="file" accept="image/*" class="hidden"
                                        @change="
