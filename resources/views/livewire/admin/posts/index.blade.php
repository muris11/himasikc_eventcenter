<div>
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-8">
        <div class="animate-fade-in-up">
            ...
            <p class="text-text-muted mt-1 text-lg">Kelola semua postingan blog dengan mudah dan efisien</p>
        </div>
    ...
    <div class="bg-surface rounded-2xl border border-border shadow-sm p-6 mb-8 hover:shadow-xl hover:shadow-primary-500/5 transition-shadow duration-300">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="md:col-span-2">
                <label for="search" class="block text-sm font-bold text-text-main mb-2">Cari Postingan</label>
        ...
        @if ($search || $statusFilter)
            <div class="mt-6 flex flex-wrap items-center gap-2 pt-4 border-t border-border animate-fade-in">
                ...
                @if ($statusFilter)
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-primary-50 text-primary-700 border border-primary-100">
                        Status: {{ $statusFilter === 'published' ? 'Dipublikasikan' : 'Draf' }}
                        ...
                        <tr class="hover:bg-surface-secondary/30 transition-colors duration-200 group">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-4">
                                        <div class="text-sm font-bold text-text-main group-hover:text-primary-600 transition-colors">{{ $post->title }}</div>
                                        <div class="text-xs text-text-muted mt-0.5">{{ $post->created_at->format('d M Y') }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-text-main">{{ $post->user->name ?? 'Tidak Diketahui' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $post->is_published ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-yellow-50 text-yellow-700 border border-yellow-100' }}">
                                    ...
                                    {{ $post->is_published ? 'Dipublikasikan' : 'Draf' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.posts.edit', $post) }}"
                                        class="h-11 w-11 flex items-center justify-center bg-surface border border-border text-text-muted hover:text-primary-600 hover:border-primary-200 hover:bg-primary-50 rounded-xl transition-all shadow-sm hover:shadow-md" title="Edit">
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-16 text-center">
                                <div class="animate-fade-in-up">
                                    ...
                                    <h3 class="text-xl font-bold text-text-main mb-2 font-heading">Belum ada Postingan</h3>
                                    <p class="text-text-muted mb-6 max-w-sm mx-auto">Mulai dengan membuat postingan pertama Anda untuk dibaca oleh pengguna.</p>
                                    <a href="{{ route('admin.posts.create') }}"
        ...
        <div class="md:hidden grid grid-cols-1 divide-y divide-border">
                ...
                <div class="p-4 hover:bg-surface-secondary/30 transition-colors">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="shrink-0 h-16 w-16 rounded-lg overflow-hidden border border-border bg-surface-secondary">
                            @if ($post->image_path)
                                <img class="h-full w-full object-cover" src="{{ asset('storage/' . $post->image_path) }}" alt="">
                            @else
                                <div class="h-full w-full flex items-center justify-center text-text-muted">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex justify-between items-start">
                                <h3 class="text-sm font-bold text-text-main line-clamp-2 mb-1">{{ $post->title }}</h3>
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium {{ $post->is_published ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-yellow-50 text-yellow-700 border border-yellow-100' }}">
                                    {{ $post->is_published ? 'Dipublikasikan' : 'Draf' }}
                                </span>
                            </div>
            ...
            @empty
                <div class="p-8 text-center">
                    ...
                    <p class="text-text-muted text-sm font-medium">Belum ada Postingan</p>
                </div>
            @endforelse
        </div>
